...
load_pubkey() {
  ...
  echo -n ${PRIVATE_KEY} > $private_key_path
...
configure_git_ssl_verification() {
  skip_ssl_verification=${SKIP_SSL_VERIFICATION-false}
...
configure_credentials() {
  local username=${ACTION_USERNAME-${GITTAR_USERNAME}}
  local password=${ACTION_PASSWORD-${GITTAR_PASSWORD}}
...
load_git_crypt_key() {
  ...
  echo -n ${GIT_CRYPT_KEY} > $git_crypt_tmp_key_path
