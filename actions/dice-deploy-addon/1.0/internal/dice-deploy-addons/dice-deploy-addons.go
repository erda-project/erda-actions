...
func Deploy(conf *dice_deploy_services.Conf) (*dice_deploy_services.DeployResult, error) {
	...
	err := retry.DoWithInterval(func() error {
		r, err := httpclient.New(httpclient.WithCompleteRedirect()).Post(conf.DiceOpenapiPrefix).Path(fmt.Sprintf("/api/deployments/%s/actions/deploy-addons", conf.DeploymentID)).
