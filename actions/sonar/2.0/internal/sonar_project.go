...
func isProjectExistedError(err error) bool {
	return strings.Contains(err.Error(), "Could not create Project, key already exists:")
}
...
func (sonar *Sonar) createProject(projectKey string) error {
	...
	httpResp, err := httpclient.New(httpclient.WithCompleteRedirect()).BasicAuth(sonar.Auth.Login, sonar.Auth.Password).
		...
		Do().Body(&respBody)
	if err != nil {
		if httpResp.StatusCode() == http.StatusBadRequest && isProjectExistedError(err) {
			logrus.Infof("Project already existed, projectKey: %s", projectKey)
			return nil
		}
		return err
	}
