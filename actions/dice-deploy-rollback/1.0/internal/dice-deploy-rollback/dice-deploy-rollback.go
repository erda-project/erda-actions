...
type conf struct {
	...
	DeploymentID uint64 `env:"ACTION_DEPLOYMENT_ID"`
}
...
func prepareRequest(conf *conf) *deployRequest {
	...
	req.DeploymentID = conf.DeploymentID
	return req
}

func (d *dice) Deploy(req *deployRequest, conf *conf) (*DeployResult, error) {
	...
	err := retry.DoWithInterval(func() error {
		r, err := httpclient.New(httpclient.WithCompleteRedirect()).Post(conf.DiceOpenapiPrefix).
			Path(fmt.Sprintf("/api/runtimes/%s/actions/rollback-action", conf.RuntimeID)).
