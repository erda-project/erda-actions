...
$JAVA_HOME/bin/java $JAVA_OPTS -jar $CATALINA_OPTS -Djava.security.egd=file:/dev/./urandom   -Dcatalina.logs=$CATALINA_HOME/logs -Dpandora.location=$CATALINA_HOME/deploy/taobao-hsf.sar   "/home/admin/app/${APP_DIR}/app.jar"  --spring.profiles.active=$SPRING_PROFILES_ACTIVE  --server.context-path=/  --server.tomcat.uri-encoding=UTF-8 --server.tomcat.max-threads=400
