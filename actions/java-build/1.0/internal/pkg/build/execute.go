...
func build(cfg conf.Conf) error {
	...
	runCommand(fmt.Sprintf(" cp %s %s ", "/opt/action/comp/spot-agent/${DICE_VERSION}/spot-agent/spot-agent.jar", fmt.Sprintf("%s/%s/%s/spot-agent.jar", cfg.WorkDir, pwdName, "spot-agent")))
