...
## （已废弃）配合 release 的 services 使用

Warning!!! `java-build` 配合 `release` 构建的方式已经废弃，未来会移除，请及时切换到 `java` action。

```yaml
 services:
  # 声明 service 的名称，名称对应 dice,yaml 中的 service
  java-demo:
   # 运行环境镜像
   image: openjdk:17-ea-3-jdk-buster
   # 项目运行的命令, ${java-build:OUTPUT:JAVA_OPTS} 是对应的监控命令，固定加上即可
   cmd: java ${java-build:OUTPUT:JAVA_OPTS} -jar /target/app.jar
   copys:
    # 将当前 java-build 生成的 jar 包拷贝到当前 /target 下，这样就可以配合 java -jar 直接运行
    - ${java-build:OUTPUT:buildPath}/target/app.jar:/target/app.jar
